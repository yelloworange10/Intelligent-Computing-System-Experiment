...
class MNIST_MLP(object):
    ...
    def build_model(self, batch_size=100, input_size=784, 
                    hidden1=256, hidden2=256, out_classes=10): # 建立网络结构
        hidden1 = 256
        hidden2 = 256
        ...
        # FC2
        input_shapem2 = output_shapem1  # output of first layer is input to second
        weight_shapem2 = pycnnl.IntVector([batch_size, 1, hidden1, hidden2])
        output_shapem2 = pycnnl.IntVector([batch_size, 1, 1, hidden2])
        self.net.createMlpLayer('fc2', input_shapem2, weight_shapem2, output_shapem2)
        self.net.createReLuLayer('relu2')

        # FC3
        input_shapem3 = output_shapem2
        weight_shapem3 = pycnnl.IntVector([batch_size, 1, hidden2, out_classes])
        output_shapem3 = pycnnl.IntVector([batch_size, 1, 1, out_classes])
        self.net.createMlpLayer('fc3', input_shapem3, weight_shapem3, output_shapem3)
        self.net.createReLuLayer('relu3')

        output_shape = pycnnl.IntVector(3)
        output_shape[0] = batch_size
        output_shape[1] = 1
        output_shape[2] = out_classes

        self.net.createSoftmaxLayer('softmax',output_shape , 1)  # axis=1 for softmax along classes dimension
    ...
    def load_data(self, data_path, label_path):
        ...
        test_images = self.load_mnist(data_path, is_images=True)
        test_labels = self.load_mnist(label_path, is_images=False)
